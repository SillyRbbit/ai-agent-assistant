...
The completed [bounded plan](docs/plans/2026-09-22-configurable-agent-current-models.md)
replaces the removed GPT-5.4 choices with GPT-5.6 Luna, Terra and Sol, and adds
one append-only stored-profile migration. The exact successor scope is 21 paths;
the cumulative candidate is 38 paths. Focused and full offline validation plus
completion gates passed. Zero live attempts remain. No publication or
D-125/M1/M2 work is authorized.

## Configurable agent demo — 2026-09-22

The owner selected one coherent implementation, with independently scoped
connection limitations and in-task routine repairs. The
[completed plan](docs/plans/2026-09-22-configurable-agent-demo.md) covers Agents,
persistent settings/private notes, bounded advice, offline validation and one
native walkthrough. Historical diagnostics/closeouts are preserved evidence,
not prerequisites. Full offline verification and the native save/restart/
simulation walkthrough passed. OpenAI live remains owner-setup pending at 0/1;
Codex remains disabled with 0/1 unused. D-125/M1/M2 remain parked.
...
[bounded plan](docs/plans/2026-09-21-direct-provider-diagnostics-evidence-closeout-validator-retry.md)
records the ten-path repository delta and 44-path cumulative inventory. It kept
the external Git helper byte-oriented, retained absent-path `None` handling, and
used `b"node_modules"` for the one membership assertion. The completion evidence
contains no repeated application test or build; all live attempts remain exhausted.

## Direct provider diagnostics evidence closeout retry — 2026-09-21

The active documentation-only retry preserves the 40-path closeout candidate and
corrects only its verification preconditions. See the bounded plan at
docs/plans/2026-09-21-direct-provider-diagnostics-evidence-closeout-retry.md.
The exact retry delta is ten documentation paths and the cumulative inventory is
42 paths. Existing local formatter tooling was clone-copied without installation.
The new preservation validator raised TypeError while comparing status output, so
the retry is terminal failed without repair or retry.

## Direct provider diagnostics evidence closeout — 2026-09-21

The active documentation-only successor preserves the 38-path diagnostics
candidate and corrects only its completion evidence. See the
[bounded plan](docs/plans/2026-09-21-direct-provider-diagnostics-evidence-closeout.md).
The exact successor delta is ten documentation paths and the cumulative inventory
is 40 paths. No application tests or builds are repeated; historical passing
results remain evidence of the inherited candidate. This successor is terminal
failed because the documentation command lacked `prettier` and its initial
preservation validator failed on an allowed new path. No repair or retry occurred.

## Direct provider stream diagnostics message assertion — 2026-09-21

The active isolated successor corrects only the UI regression table's
policy-message expectation while preserving the terminal-failed predecessor.
See the [bounded plan](docs/plans/2026-09-21-direct-provider-stream-diagnostics-message-assertion.md).
The exact successor delta is 11 paths and the cumulative inventory is 38 paths.
All production and dependency bytes remain inherited unchanged. Focused and
complete offline validation passed, with exact scope and predecessor preservation.
Ordinary completion failed on a noncanonical report heading, so the increment is
terminal failed without repair or retry. No live request or publication is authorized.
...
Reuse the existing V0-13 plan for this owner's superseding direct-demo contract.
Its historical gateway milestones are preserved; this implementation does not
complete those milestones or admit real-content-v2. Ordinary admission succeeded
for `personal-assistant-direct-implementation`; completion is evidence-bound.
