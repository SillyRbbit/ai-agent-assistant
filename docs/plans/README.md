...
Repository Workflow Increment trusted self-hosted runner routing is active under [`repository-self-hosted-runner.md`](repository-self-hosted-runner.md). It changes repository automation and policy only; local verification is required before publication and successful execution on the registered runner is required before completion.
...
Increment 4V bind initial terminal approval audit is verified at `3440ce9` on open PR #23 under [`04v-bind-initial-terminal-approval-audit.md`](04v-bind-initial-terminal-approval-audit.md). Its exact source/test and closeout scope retain a valid `04v` marker. Publication waits for usable remote checks after the self-hosted runner workflow is merged; it adds no durable persistence, runtime coordination, transport, dispatch, or execution.
