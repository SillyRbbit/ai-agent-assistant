...
- Phase 1 authentication uses a system browser and OAuth 2.0 Authorization Code Flow with PKCE behind a pluggable OAuth/OIDC application boundary. Microsoft, Google, and Apple remain unselected candidates. The configured provider determines the issuer; the gateway must validate the trusted issuer, audience, signature, expiration, applicable tenant, and authorization context. Provider neutrality never permits caller-selected issuers, endpoints, audiences, client identities, tenant rules, or origins.
...
- A future desktop gateway access token must be short-lived with a maximum 15-minute lifetime, audience-bound, read only by trusted Rust through the platform secret-store abstraction, and stored in process memory. Any refresh or session credential must use platform-secure credential storage: macOS Keychain on macOS and an equivalent reviewed facility on a future supported platform. The WebView must never receive either credential.
