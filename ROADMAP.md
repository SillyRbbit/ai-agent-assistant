...
| Personal Assistant text-only v0                                       | Active; V0-1 complete locally | V0-1 seals the fixed synthetic empty-tool Rust request/runtime/host without transport or UI                                                                      | Owner acceptance/publication of V0-1; V0-2..V0-14 lifecycle, auth, provider, HTTPS, Tauri, ZDR, operations, and target-Mac gates remain Blocked |
...
is locally verified complete from synchronized `main` baseline `0b22ee7`. It
adds only a sealed fixed synthetic Rust request/runtime branch and no-input
volatile host; there is no response ingress, UI, IPC, provider, network,
credential, persistence, tool, filesystem, or device effect. The exact diff is
uncommitted and unpublished pending owner review.

The [`Personal Assistant v0 program`](docs/plans/2026-08-28-personal-assistant-v0-program.md)
still records D-094's full sequence. V0-2 remains Blocked until V0-1 is
published or otherwise accepted as the exact source baseline. V0-3 through
V0-14 remain Blocked across lifecycle, identity, JWT/JWKS, no-traffic
