...
V0-1 is published at `dca584e`; V0-2 is locally verified complete with `PASS
WITH ADVISORIES` from baseline `8e382e8`. No successor is Ready.
...
| Personal Assistant text-only v0                                       | Active; V0-2 complete locally | V0-1 seals the fixed synthetic empty-tool request/runtime; V0-2 adds a volatile bounded session journal, deadline, cancellation, and presentation owner without transport or UI | Owner review/publication of V0-2; V0-3 is blocked by D-076/TS-017 and all external, Tauri, ZDR, provider, and real-content gates remain blocked |
...
persistence, tool, filesystem, or device effect. The exact diff is uncommitted
