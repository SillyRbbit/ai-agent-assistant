...
impl<R: AgentRuntime> AgentOrchestrator<R> {
    ...
    fn validate_infrastructure_operations_start(
        &self,
        source_context: &AgentExecutionContext,
        attribution: &AgentAttribution,
    ...
    fn prepare_infrastructure_operations_child(
        &self,
        agent_id: AgentId,
        ordinal: u8,
        objective: &str,
    ...
    fn consume_root_for_infrastructure_start(
        &mut self,
    ...
    fn record_infrastructure_continuation_failure(
        &mut self,
    ...
    fn terminalize_infrastructure_child_failure(
        &mut self,
        task_id: &AgentTaskId,
        task_code: AgentTaskFailureCode,
        workflow_code: InfrastructureOperationsPartialFailureCode,
    ...
    fn start_infrastructure_synthesis(&mut self) -> AgentOrchestratorResult<AgentExecutionContext> {
        let root_id = self
            .root_task_id
            .clone()
            .ok_or(AgentOrchestratorError::RootMissing)?;
    ...
    fn preflight_infrastructure_operations_event_capacity(
        &self,
        task_id: &AgentTaskId,
        event: &UntrustedRuntimeEvent,
    ) -> AgentOrchestratorResult<()> {
    ...
    fn record_infrastructure_operations_event_acceptance(&mut self, task_id: &AgentTaskId) {
    ...
    fn prepare_infrastructure_operations_terminal(
        &self,
        task_id: &AgentTaskId,
        event: &UntrustedRuntimeEvent,
        ...
        let terminal = match (workflow, workflow.phase(), event) {
            (
                ...
                let (task_outcome, first, failure) = match event {
                    ...
                    {
                        (
                            AgentTaskOutcome::Cancelled(super::task::AgentTaskCancellation::new(
                                task_id.clone(),
                                AgentId::CloudInfrastructure,
                            )),
                            InfrastructureOperationsFirstOutcome::Cloud(
    ...
    fn apply_prepared_infrastructure_operations_terminal(
        &mut self,
        task_id: &AgentTaskId,
    ...
    fn start_infrastructure_security(&mut self) -> AgentOrchestratorResult<AgentExecutionContext> {
    ...
    fn infrastructure_first_task_id(&self) -> Option<AgentTaskId> {
        self.infrastructure_operations
            .as_ref()
    ...
    fn infrastructure_qa_task_id(&self) -> Option<AgentTaskId> {
        self.infrastructure_operations
            .as_ref()
            .map(|workflow| match workflow {
                InfrastructureOperationsWorkflowState::Cloud(value) => &value.audit_contexts,
    ...
    fn infrastructure_security_task_id(&self) -> Option<AgentTaskId> {
        self.infrastructure_operations
            .as_ref()
            .map(|workflow| match workflow {
                InfrastructureOperationsWorkflowState::Cloud(value) => &value.audit_contexts,
    ...
    fn cancel_root(
        ...
        let infrastructure_cancel_stage = self
            .infrastructure_operations
            .as_ref()
            .filter(|workflow| {
                !matches!(
        ...
        if infrastructure_cancel_stage.is_some() {
            let workflow = self
                .infrastructure_operations
                .as_ref()
                .ok_or(AgentOrchestratorError::InfrastructureOperationsWorkflowMissing)?;
    ...
    fn cancel_infrastructure_child(
        &mut self,
        child_task_id: &AgentTaskId,
        root_is_cancelling: bool,
    ) -> AgentOrchestratorResult<AgentTaskCancellationOutcome> {
...
fn push_cloud_infrastructure_transition(
...
fn push_systems_operations_transition(
...
fn infrastructure_synthesis_predecessor_from_state(
