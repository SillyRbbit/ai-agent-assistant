...
The configurable demo's former GPT-5.4 choices are corrected to the
documented GPT-5.6 Luna, Terra and Sol catalog, with Luna as default. Migration
4 upgrades only stored OpenAI 5.4 profiles, increments revision and preserves
other profile fields, with a safe saturated-maximum edge. Focused and full
offline verification and completion gates pass. The prior optional live request ended in sanitized
`model_unavailable`, with no completed answer or retry. Zero live attempts
remain. D-128/D-127 advisories remain and D-125/M1/M2 stay parked.

## Configurable agent demo — 2026-09-22

The current owner-approved task implements nine independently persisted native
agent preferences and private notes, plus bounded text advice. Native request
assembly binds settings and note ownership; later revisions require a fresh
conversation. Codex selection remains live-disabled because complete isolation
is not verified for the installed runtime. Direct OpenAI and simulation use one
native conversation path with no tools or fallback.

Full offline verification passed. The native walkthrough directly observed
settings/note save, process restart persistence, simulation streaming, explicit
completion and released native generation ownership. This completes the usable
offline demo, not a live-provider success claim. No live request was made;
OpenAI remains private-owner-setup pending at 0/1 and Codex remains disabled at
0/1 because complete tool/file isolation is unverified. Historical exhausted
requests and failures remain unchanged. D-128/D-127 advisories remain;
D-125/M1/M2 are parked. See the
[plan](docs/plans/2026-09-22-configurable-agent-demo.md).

## Direct provider stream stage live-result evidence closeout — 2026-09-21

An explicitly authorized documentation-only successor completed at
`/private/tmp/cortexa-direct-provider-stream-stage-live-result-evidence-closeout`,
detached from `ebaae34ea32e4930e60b53c6064a8bf25036d52a`. The completed 17-path
candidate transferred byte-for-byte before ordinary admission. The new exact
ten-document delta yields 19 cumulative changed paths and does not modify
runtime, tests, SECURITY.md, dependencies, configuration, workflows, hooks,
skills or harnesses.

Direct Computer Use verified the idle, enabled and disclosed pre-request state
but did not observe the post-click result after its binding became inactive. The
owner-supplied screenshot separately shows a terminal
`provider_stream_error_event`, no completed answer and the static no-automatic-
retry statement. The final request was consumed; zero authorized attempts
remain. Upstream cause, earlier transient output and live ownership release are
not established. Native live success remains unverified. D-128 and D-127 remain
advisories, and D-125/M1/M2 stay parked.
Documentation, repository, security, whitespace, preservation, session,
independent-review, report-schema, finalization and Stop evidence passed.

## Direct provider stream stage diagnostics evidence closeout — 2026-09-21

The documentation-only successor completed from tree-identical main baseline
`ebaae34ea32e4930e60b53c6064a8bf25036d52a`. It inherited the stream-stage
diagnostics candidate whose application checks and debug bundle passed, while
the predecessor completion report remained terminal `FAIL / Blocked` after its
schema-validation failure. This successor has no application behavior scope:
it established separate documentation/evidence completion while retaining
the unused native request, existing native GUI/live-success advisories, D-128
custody/abort limits, D-127 audit debt and parked D-125/M1/M2 work.
...
The separately admitted documentation-only successor completed from detached
baseline `0ed15810e90b6a4bd312a8096c61b0abb1ab7eff`. It inherited the terminal
42-path retry candidate, clone-copied existing formatter tooling locally, and
changed ten documentation paths for a 44-path cumulative inventory. The external
preservation validator retains its absent-path-safe `None` return and compares
bytes for `node_modules`. The completion record has passing documentation,
repository, security, whitespace, preservation, session, quality and report
evidence. Native live success and remaining GUI evidence remain advisories; all
three approved live attempts are exhausted. D-125/M1/M2 remain parked.

## Direct provider diagnostics evidence closeout retry — 2026-09-21

An authorized documentation-only retry is active at
/private/tmp/cortexa-direct-provider-diagnostics-evidence-closeout-retry.
It inherited the exact 40-path closeout candidate without gate state and uses an
APFS clone of existing local formatter tooling. No executable, dependency,
configuration, workflow, hook, skill or harness byte changed. Historical
application evidence remains 18 focused frontend tests, 43 focused Rust tests
and full offline verification passed; no application check is rerun. The new
preservation validator raised TypeError while comparing status output, so this
retry is terminal failed without repair or retry. Native live success remains
unverified, all live attempts are exhausted and D-125/M1/M2 stay parked.

## Direct provider diagnostics evidence closeout — 2026-09-21

An explicitly authorized documentation-only successor is active at
`/private/tmp/cortexa-direct-provider-diagnostics-evidence-closeout`. It copied
and byte-verified the terminal 38-path candidate without its gate state, then
received ordinary admission. The only new work is completion evidence and
superseding current-state documentation. Historical application validation remains
18 focused frontend tests, 43 focused Rust tests and full offline verification,
all passed; no runtime validation is repeated. This successor is terminal failed:
local `prettier` was unavailable and the first preservation validator did not
handle successor-only files. Native live success remains unverified, all three
live attempts are exhausted, existing advisories remain and D-125/M1/M2 stay parked.

## Direct provider stream diagnostics message assertion — 2026-09-21

An explicitly authorized isolated corrective successor is admitted at
`/private/tmp/cortexa-direct-provider-stream-diagnostics-message-assertion`.
It inherited and byte-verified the exact 36-path failed candidate without
copying raw gate state. The predecessor remains valid `FAIL / Blocked` and is
not reopened, rewritten or promoted.

The one executable change makes each parameterized UI row state its expected
policy-message suffix. All other assertions and production, dependency, Rust,
client-test and component bytes remain unchanged. Focused frontend (18), focused
Rust (43), and complete offline verification passed. All three live attempts
remain exhausted. Ordinary completion failed on the report's noncanonical scope
heading; this successor is terminal failed without repair or retry. Native live
success remains unverified, existing advisories remain, and D-125/M1/M2 stay parked.
...
Current capability: fixed synthetic text can traverse the development-only native
adapter through the existing deterministic runtime. This statement describes
implemented and locally testable behavior, not account access or a successful
paid request. Browser-only mode remains unavailable for native traffic. The
separate Research/Knowledge native GUI advisory remains open.
...
plain group headers and nine equal-height agent cards. Card widths,
orchestrator sizing, graph renderer and shared interactions remain.
This is presentation cleanup, not a connected native Graph or live assistant.

The owner-approved candidate starts at verified remote main
`f176c36cc701b5a296162331cfcb2600b2157663` on
`codex/demo-cleanup-publication`. Its exact scope is 13 UI changes and nine
documentation paths. Admission, 21 baseline tests, 191 affected candidate
tests, lint, typecheck, build, desktop/narrow browser, documentation,
repository, security, whitespace, scope/preservation and session checks passed.
Quality: PASS WITH ADVISORIES, as recorded in the
[publication report](docs/reviews/2026-09-20-demo-cleanup-publication-post-increment-review.md).
Native GUI smoke remains advisory. The ordinary gate must validate before
commit/push/PR; this report does not claim remote checks or merge completion.

Seven unpublished local harness commits and unfinished D-125 work remain in
the original checkout. Their historical failures and blocked outcomes are not
rehabilitated by this separate candidate. M1/M2 and live operational work
remain parked or blocked.
...
Current capability remains separate frontend fixtures, a native manually
stepped content-free lifecycle, and a Conversations mock. The connected
Graph/brief is proposed, not implemented. Immediate dependency: exact browser
runner/runtime acquisition approval and clean-baseline owner Git disposition.
...
synchronized `main` at
`0931df66c389bdc13c705d1259706c4d3770761c`. Xcode 26.6 created and listed one
Developer ID Application certificate record after the owner privately
confirmed Account Holder authority. Sanitized CLI checks reported zero matching
certificates and zero usable code-signing identities for the current macOS
user. The owner later confirmed categorically that Keychain Access shows the
certificate with a private key beneath it. The owner then supplied the exact
residual-risk confirmation and approved one `keychain_identity_v1` run. The
single scoped query returned
`passed_one_label_matched_valid_codesigning_identity`, establishing current
visibility of one matching valid identity. Non-exported owner control and
signing remain unconfirmed. The owner reported no authorization prompt and no
visible state change.

The increment stopped closed with `FAIL`; D-097 records a valid terminal failed
state and its post-increment completion marker remains absent. No product
source, dependency, configuration, entitlement, capability, GitHub Actions
workflow, credential, fake Keychain item, signing, notarization,
distribution, provider, network, filesystem, or device-effect path changed.
The only executable source change is the repository governance hook and its
focused tests under D-097.
Private screenshots crossed the planned chat evidence boundary, but no private
identifier or signing material entered Git. V0-3 remains Blocked, and no retry
or further discrepancy/signing execution is approved.

On 2026-08-29, the owner initially selected documentation drafting for a future
sanitized, Keychain-scoped local-identity discrepancy check under the active
recovery gate. The
[draft plan](docs/plans/2026-08-29-v0-keychain-scoped-identity-discrepancy.md)
defines closed output, bounded in-memory parsing, no raw evidence, exact stop
conditions, and zero retries. Static review corrected disclosures for possible
OS trust traffic/cache/log/state, local process-metadata visibility,
identity-metadata scope, and the sole permitted quoted here-document. The owner
later accepted those exact boundaries, confirmed personal control of the target
Mac, and approved the single passing run recorded above. No raw identity data,
signing, certificate/key mutation, source, or product capability was added.
Post-correction static architecture/security review and every required
documentation-tier check Passed before execution.
The required stop-hook post-increment workflow reran full verification and all
documentation/security gates successfully; npm audit reported 0
vulnerabilities, and the one existing opt-in Hermes probe remained intentionally
ignored. The historical non-export criterion cannot Pass as written, its
prospective evidence standard is accepted under D-096, signing is Not run, and
the historical screenshot/privacy failure remains Failed; therefore the result
stays `FAIL` and no completion marker is finalized.

The owner next authorized a documentation-only draft and then approved its
evidence-standard milestone for the
...
The review found that current pairing, identity-list, and signature evidence
cannot prove historical absence of export or exclusive custody; the separate
current-item extractability attribute was not queried and remains `not_proven`.
Accepted D-096 now governs those prospective claims with bounded owner
attestation, `approved_workflow_private_key_export=not_performed`, future
present-session key use, and fixed `not_proven` categories for technical non-
...
cache/socket/log boundaries that were not separately disclosed before execution.
No account field was emitted and remote traffic is not proven; acceptance is
Pending and no rerun is authorized. The earlier hook could not represent a
terminal `FAIL`; D-097 and its same-active-gate implementation now preserve the
immutable result as valid `failed` / `FAIL` / `Blocked` without a completion
marker. Representation is resolved, but Blocked readiness still denies a
successor. Environment/cache routing also does not confine executable build
scripts, which remains an additional operational-signing blocker.

The final stop-hook recheck corrected report-only schema drift: three finding
categories now use the hook's closed allowlist, the template-required scope
section is present, and the parent plan's superseded operation boundary is
explicitly historical. The hook's read-only validator now reaches only the
expected blocking-evidence result. The later D-097 implementation adds only the
terminal failed workflow state and focused gate tests. It changes no `FAIL`,
Pending, Failed, or Not-run evidence and creates no completion or product
authority. Future signing also remains Blocked until account-directory
resolution is contained or separately disclosed and accepted in the exact
sanitizer contract and successor readiness is truthfully unblocked.
